...
category: posts
comments: true
tags: [Sudev Ambadi, Sudev, unix command line, command line text manipulation, data science and unix commandline, data science large files, large dataset]
---
...
Let's assume that we have a BIG csv with many coloumns and we are interested in the sum of fourth coloumn of the file. 

{% highlight bash %}
cat big.csv | awk -F "," '{ sum += $4 } END { printf "%.2f\n", sum}'
{% endhighlight %}
...
1. With Awk 
	...
	* `sum += $4` will increment the variable sum with fourth coloumn of each line.
...
{% highlight bash %}
# To get the first 5 lines of your csv 
head big.csv -n 5
# Similarly for the last 5 lines 
tail big.csv -n 5 
{% endhighlight %}
We can even save the sample to a separate file to view them using a editor or to run some script over the sample file. Using the redirect operator `>` in bash yuo can redirect the output to a file.
{% highlight bash %}
# first 100 lines into a new file first100.txt
head -n 100 > first100.txt
{% endhighlight %}
...
*Grep*    
We wish to search for the line containing some text `audi` we can use grep to search and return those rows for you. Grep supports regular expression and hell lot of options.

{% highlight bash %}
# Search for text audi 
grep audi big.csv
# subset data with only lines containing audi 
grep audi big.csv > audi.csv
{% endhighlight %}
...
{% highlight bash %}
# Replace all occurence of "audi" to Audi in your data
sed -i s/audi/Audi/g big.csv
{% endhighlight %}
Read more about [Sed](https://en.wikipedia.org/wiki/Sed) and [Awk](https://en.wikipedia.org/wiki/Awk).     
<br />
*Sort and uniq*   
Using sort command will sort the csv treating each line as a single string, we can use sort to even sort based on a coloumn, numerical order or in reverse order. Uniq can be used to return only uniq rows or return the duplicated ones. Assume that we have a csv as shown below:
{% highlight bash %}
...
<br />
I just want to say that working with unix command line tools is easier than using any other graphical tools, all you have to do is use them for once.    
*And you dont have to remember any of these parameters, all you have to do is to search through their man page.*
<br />
Links:

* [Using command line for datascience](http://jeroenjanssens.com/2013/09/19/seven-command-line-tools-for-data-science.html)
* [Noufal Ibrahim's](http://thelycaeum.in/blog/2013/09/03/text_processing_in_unix/)
* [I got inspired to write this from here](http://www.gregreda.com/2013/07/15/unix-commands-for-data-science/)
